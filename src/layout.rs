use web_sys::{
    js_sys::Function, wasm_bindgen::{prelude::Closure, JsCast}, Element, HtmlElement
};
use yew::prelude::*;

const X_LINK: &str = env!("X_URL");
const PRIMAL_LINK: &str = env!("PRIMAL_URL");
const MEDIUM_LINK: &str = env!("MEDIUM_URL");
const GITHUB_LINK: &str = env!("GH_URL");
...
pub fn app_header() -> Html {
    html! {
        <div
            class="flex flex-row sm:flex-col items-center gap-8 z-50 bg-nostr-light rounded-lg sm:fixed sm:top-1/2 sm:right-0 mx-8 p-2 sm:py-8 sm:px-2">
...
pub fn app_footer() -> Html {
    html! {
        <div class="w-full flex flex-col items-center z-10 text-center rounded-t-full bg-nostr-light py-12">
            <h2 id="contact-us">{"Follow Us"}</h2>
            <div class="flex flex-row items-center gap-8 text-center mt-4 text-base sm:text-lg">
                <a target="_blank" href={X_LINK} class="hover:text-nostr-dark flex flex-col items-center">
                    <img src="/public/assets/x.png" alt="Twitter Icon" class="h-8 sm:h-12 md:h-16" />
                    <p>{"@illuminodes"}</p>
                </a>
                <a target="_blank" href={PRIMAL_LINK} class="hover:text-nostr-dark flex flex-col items-center">
                    <img src="/public/assets/nostr.png" alt="Nostr Icon" class="h-8 sm:h-12 md:h-16" />
                    <p>{"Nostr"}</p>
                </a>
                <a target="_blank" href={MEDIUM_LINK} class="hover:text-nostr-dark flex flex-col items-center">
                    <img src="/public/assets/medium.png" alt="Medium Icon" class="h-8 sm:h-12 md:h-16" />
                    <p>{"Medium"}</p>
                </a>
                <a target="_blank" href={GITHUB_LINK} class="hover:text-nostr-dark flex flex-col items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 sm:h-12 md:h-16 stroke-white" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                        <path
                            d="M5.315 2.1c.791 -.113 1.9 .145 3.333 .966l.272 .161l.16 .1l.397 -.083a13.3 13.3 0 0 1 4.59 -.08l.456 .08l.396 .083l.161 -.1c1.385 -.84 2.487 -1.17 3.322 -1.148l.164 .008l.147 .017l.076 .014l.05 .011l.144 .047a1 1 0 0 1 .53 .514a5.2 5.2 0 0 1 .397 2.91l-.047 .267l-.046 .196l.123 .163c.574 .795 .93 1.728 1.03 2.707l.023 .295l.007 .272c0 3.855 -1.659 5.883 -4.644 6.68l-.245 .061l-.132 .029l.014 .161l.008 .157l.004 .365l-.002 .213l-.003 3.834a1 1 0 0 1 -.883 .993l-.117 .007h-6a1 1 0 0 1 -.993 -.883l-.007 -.117v-.734c-1.818 .26 -3.03 -.424 -4.11 -1.878l-.535 -.766c-.28 -.396 -.455 -.579 -.589 -.644l-.048 -.019a1 1 0 0 1 .564 -1.918c.642 .188 1.074 .568 1.57 1.239l.538 .769c.76 1.079 1.36 1.459 2.609 1.191l.001 -.678l-.018 -.168a5.03 5.03 0 0 1 -.021 -.824l.017 -.185l.019 -.12l-.108 -.024c-2.976 -.71 -4.703 -2.573 -4.875 -6.139l-.01 -.31l-.004 -.292a5.6 5.6 0 0 1 .908 -3.051l.152 -.222l.122 -.163l-.045 -.196a5.2 5.2 0 0 1 .145 -2.642l.1 -.282l.106 -.253a1 1 0 0 1 .529 -.514l.144 -.047l.154 -.03z"
